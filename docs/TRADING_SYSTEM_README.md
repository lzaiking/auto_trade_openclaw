...
- `code/build_monthly_factors.py`：把本地价格缓存和公开宏观数据整合成 `monthly_factors.csv`
...
- `report/prediction_metrics.json`：规则预估准确度，包含 30 自然日端点价 AUC 和窗口内交易日均价 AUC
- `report/model_prediction_metrics.json`：线性模型预估准确度
- `report/model_feature_importance.json`：线性模型标准化系数重要度和常识备注
...
### 1) 预测模块
- 输入因子：当前只保留可复现的 `breadth_200dma` 和 FRED `DTB3` 3 个月 T-bill 利率；PE/PB/EPS/FCF 等缺少可靠历史点位数据的字段先不进入训练表。
- 价格数据：`monthly_factors.csv` 同时保存 QQQ/GLD/SGOV 的月度 close、200 日均线偏离、14 日 RSI 和 63 日涨幅。
- 后验 label：当前日期往后 30 个自然日，若当天休市则取之后第一个可用交易日作为端点；均价口径使用这段自然日窗口内所有实际交易日价格均值。
- 输出：`predicted_1m_return`、0-1 上涨分数、端点价后验收益、均价后验收益、误差和模型系数。
- 规则预估是当前主策略输入；线性模型预估作为 benchmark 保留，优先使用 `sklearn.linear_model.RidgeCV`，没有 sklearn 时回退到纯 Python ridge。
- QQQ 的线性模型 score 固定使用预测收益组件，评估重点是 QQQ 端点 AUC 和 30 自然日窗口均价 AUC。
...
- 当前主策略仍使用 legacy signal 仓位映射；线性模型虽然提升了 QQQ AUC，但模型策略收益还没有稳定超过 QQQ 定投，所以暂时只作为对照场景。
- 同时输出 oracle 和 worst-case 场景，用来评估理论上限和极端错误风险。

## 因子数据

因子填写规则见 `docs/FACTOR_DATA_GUIDE.md`。重建月度数据表：

```bash
python3 code/build_monthly_factors.py
```

当前表格会整合本地价格数据和 `data/macro/DTB3.txt` 或 `data/macro/DTB3.csv`。最近日期因为还没有完整未来 30 个自然日数据，后验 label 应该留空。
